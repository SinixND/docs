...
# Best practises
## Default approach to push changes (ordered by preference)
- To save changes locally (preferred in teams):
...
### git commit
Add/Upload changes to be tracked in [history](#history)

#### Options:
| Opt | Desc 
| - | - 
| `--amend` | Modify last [commit](#commit); Needs forced [push](#push) because it modifies [history](#history)
| `-m "message"`| Add description to [commit](#commit); Omit `"message"` for interactive edit (recommended)

### git fetch
Download target [history](#history) from git; makes it available locally for further processing
...
### git rebase
Changes [history](#history) of the active [branch](#branch) by including another; result is a changed, but straight/cleaned/modified [history](#history), therefore _inherupts_ ability to backtrack
